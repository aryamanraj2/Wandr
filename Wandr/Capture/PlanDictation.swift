// PlanDictation.swift Wandr Live, on-device dictation for the capture screen. Uses SpeechAnalyzer + SpeechTranscriber (iOS 26+) rather than the legacy SFSpeechRecognizer: it runs entirely on device, streams volatile results while the host is still talking, and hands back finalized text without a round trip. The audio tap also publishes a smoothed amplitude so the orb can react to the voice rather than loop a canned animation.
...
/// Failures that are ours rather than the framework's, so they can carry a sentence worth showing instead of arriving as an opaque NSError.
...
final class PlanDictation {
    ...
    enum Phase: Equatable {
        case idle
        /// Mic granted, model warming up. The orb holds still here — a pulse would claim we're hearing something when we aren't yet.
        case preparing
        case listening
        /// Recoverable: permission refused, model unavailable, engine failure. Carries a sentence the view can show verbatim.
        case failed(String)
    }

    private(set) var phase: Phase = .idle

    /// Everything the transcriber has committed, plus anything typed. The view binds a `TextField` straight to this, so speech and keyboard edit one buffer instead of two that have to be reconciled.
    var transcript: String = ""

    /// The tail the transcriber is still revising. Rendered dimmer and never merged until it finalizes, so words don't visibly rewrite themselves.
    ...
    func start() async {
        ...
        do {
            ...
            teardown()
            // The user gets the calm sentence; the log gets the truth. Without this the underlying error was discarded at the catch, so every distinct failure — no model, no reservation, audio session refused — presented identically and none could be told apart.
            logger.error("Dictation failed to start: \(error, privacy: .public)")
            phase = .failed(Self.explain(error))
        }
    }

    /// Folds any in-flight volatile tail into the transcript before returning, so releasing the mic never drops the last few words.
    func stop() async {
        guard isListening else { return }
    ...
    private func beginTranscribing() async throws {
        ...
        // Progressive preset: volatile results as they're heard, finalized results behind them. Without it the screen sits blank mid-sentence.
        let transcriber = SpeechTranscriber(locale: locale, preset: .progressiveTranscription)
        self.transcriber = transcriber

        // `noModel` says the configuration has no model behind it, but not which half is at fault — the locale we picked, or the fact that no asset was ever installed for it. Asking the inventory directly is the only way to tell those apart, and it costs one call. Bound outside the log call: string interpolation is an autoclosure, which cannot carry an `await`.
        ...
        // Models are downloaded on demand, once per locale. Doing it here (not lazily mid-sentence) keeps the first word from being swallowed.
        if let request = try await AssetInventory.assetInstallationRequest(supporting: [transcriber]) {
            logger.log("Downloading speech model for \(locale.identifier, privacy: .public)")
            try await request.downloadAndInstall()
        }

        // Downloading the model is not the same as being allowed to use it. An app holds a small number of locale reservations, and the transcriber only produces results for a locale it has reserved — without this the pipeline starts cleanly and then stays silent, which is exactly the failure that looks like a dead microphone.
    ...
    /// Distinct causes get distinct sentences. A dead mic and an undownloaded model need different things from the user, so collapsing them into one "try typing instead" hides the only actionable part. Matched on `error.code` rather than in a `catch`: `insufficientResources` is a Swift-only `static var`, so it exists on `SFSpeechError.Code` but never as a shorthand member on `SFSpeechError` itself.
    ...
    /// Owns the engine and the audio session. It is an actor, not a set of methods on this @MainActor class, because `setCategory` and session activation take a system round trip — on the main thread that shows up as a hitch on the very tap that starts listening.
    actor Microphone {
        ...
        func start(
            ...
            // iOS 27's async pair. Activation can wait on the system audio server, so it never belongs on a synchronous path.
            _ = try await session.activate(options: [])

            let node = engine.inputNode
            let format = node.outputFormat(forBus: 0)

            // The converter resamples the mic's native format into whatever the transcriber wants. It is only ever touched inside the tap callback, which the engine serializes onto one render thread.
            nonisolated(unsafe) let converter = AnalyzerInputConverter(
                analyzerFormat: analyzerFormat ?? format
            )

            // iOS 27 deprecates this in favour of a throwing variant, but that one ships unrefined in the current SDK (its NSError** parameter imports as Void and cannot be called from Swift). Revisit once the overlay lands; the deprecated call is still fully functional.
        ...
        /// Deactivation is the call the runtime specifically warns about, so it uses the async form and stays off the main thread either way.
        func stop() async {
    ...
    /// Tears down the observable state immediately so the UI settles on the same frame as the tap, then lets the hardware wind down behind it.
    ...
    /// Asymmetric smoothing: rise fast so the orb answers a syllable on the frame it lands, fall slow so it settles instead of strobing per word.
