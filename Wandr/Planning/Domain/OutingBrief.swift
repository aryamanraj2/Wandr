// OutingBrief.swift Wandr The typed brief the whole pipeline consumes. `OutingBriefDraft` is model-generated and always uncertain. `OutingBrief` is the canonical, normalized object — every inferable value carries a `ValueSource` marker so a future "review chips" screen can show the host what Wandr guessed versus what they actually said.
...
/// Whether the night should build towards something or stay level. A birthday ends somewhere bigger than it started; a Tuesday catch-up does not. Read from what the host said, never inferred from the clock.
nonisolated enum OccasionArc: String, Sendable, Equatable, Hashable, Codable, CaseIterable {
    case flat
    case buildsToFinale
}

/// What kind of occasion this is, as four numbers the rest of the app can act on. This is the whole of what the model contributes to a plan's *shape*, and it is deliberately not a stop, a time, or a count. "A date", "five of us after work" and "her birthday" are the same arithmetic over the same venues and three different nights — the difference lives here, and nowhere in this type names a place, an hour, or a number of stops. Contrast with what came before: an occasion was a *string* (`"birthday"`) matched against a list, so every new kind of night needed a new case somewhere. Nothing here enumerates occasions. A photography walk and a first date produce different values of these four fields without either being a case anyone wrote down.
nonisolated struct OccasionProfile: Sendable, Equatable, Hashable, Codable {
    /// How fast they want to move. Drives how many stops fill a gap — via division, in `SlotSchedule`, never by the model counting anything.
    var pace: SlotSchedule.Pace
    /// 0 = the outing is a meal, 1 = the outing is an activity and food is incidental. A ranking weight, not a filter: it can never empty a deck, so it never needs relaxing and stays off the constraint ladder.
    var activityBias: Double
    /// The group has to sit down together. A hard requirement when true.
    var groupSeating: Bool
    var arc: OccasionArc

    /// What Wandr assumes when the host described no occasion at all. Mid-scale on both axes deliberately: an unspecified outing should lean neither towards a tasting menu nor towards an escape room.
    static let unspecified = OccasionProfile(
        pace: .steady, activityBias: 0.5, groupSeating: false, arc: .flat
    )

    /// Clamped on the way in — `activityBias` is the one field a model can put out of range, and a bias of 4.0 would swamp every other ranking term.
...
/// A time window that keeps "unknown" and "flexible" honest. Minutes are measured from midnight, matching the schedule surface. Actual date and time-zone resolution is a deferred rule — this step never invents one.
nonisolated struct OutingTimeWindow: Sendable, Equatable, Hashable {
    ...
    /// How long the whole outing may run, e.g. 180 for "we've only got 3 hours". A separate axis from the two clock bounds, and the reason this field exists: a duration is not a time of day. Without it, "3 hours" had nowhere to go but the clock parser, which read the bare `3` as an evening hour and produced "starts at 3 pm" — a window that keeps *every* slot and plans a whole day for a group that said they had an afternoon's worth of time.
    var maximumDurationMinutes: Int?
    /// e.g. "Friday". A label only — never parsed into a `Date` at this layer.
    var dayLabel: String?

    /// Important: a window that ends no later than it starts is **not representable**. A finish at or before the start is never a real constraint — it is a misread phrase — and the upper bound is dropped here rather than defended against downstream. This is enforced at construction because the alternative failed silently and catastrophically. `"12.00 to 5:00 pm"` parsed the `.00` as a *second* clock token, which then inherited the trailing `pm` and became a second 12:00 — so the window read as 12:00 pm → 12:00 pm, no band could fit a zero-minute span, and the host got a plan with **no stops in it at all**. A bad parse should cost the constraint, never the day. Normalising in the initializer makes the property hold for every window the app can build, from any source, rather than at the one call site that was known to be wrong.
    init(
    ...
    /// Whether the phrase produced bounds that contradicted each other, so the finish time was dropped. Drives the host-facing disclosure — a constraint given up silently is the failure mode `PlanRelaxation` exists to prevent. Deliberately excluded from `isUnknown`: a window that carries only this flag still constrains nothing, and should not switch on the "you're only free 8–9" banner.
...
/// A budget ceiling in rupees, carrying the basis the host actually stated. The basis is the whole point of this type. It used to be a bare per-head number, which meant a host who typed "2000" had that read as ₹2000 *each* — a constraint they never stated — and then compared against a venue's per-head price. Two people with ₹2000 between them were told a perfectly ordinary dinner was "over your ₹2000 limit", and the run dead-ended on it. Never a guessed precise price: either the host gave a ceiling, or there isn't one.
nonisolated enum Budget: Sendable, Equatable, Hashable {
    ...
    /// The host named a number without a basis, so it is the group's total. Assuming per head would invent the more permissive of the two readings.
    ...
    /// The ceiling to compare a venue's price against. Venue costs in the dataset are always per head, so a group total has to be divided before it means anything. This is the one place that division happens — comparing a total against a per-head price is the bug this type exists to make unrepresentable.
    ...
    /// The number the host said, whatever it meant. For display and logging only — never compare this against a venue price.
...
/// A constraint Wandr either knows, knows to be absent, or hasn't established. The three states stay distinct all the way to the UI. "Unknown" is never silently promoted to "none stated".
...
/// What the on-device extractor produces. Every field may be absent or wrong. A draft is never consumed by research, curation, or validation. It must go through `BriefNormalizing` first.
nonisolated struct OutingBriefDraft: Sendable, Equatable {
    var occasion: String?
    var timeWindow: OutingTimeWindow
    var area: String?
    var groupSize: Int?
    /// The ceiling the host named, with the basis they named it in. `nil` means they said nothing about money.
    var budget: Budget?
    var vibeTags: [String]
    var dietary: DietaryNeeds
    var accessibility: AccessibilityNeeds
    var setting: SettingPreference
    /// The stops the host asked for by name — "lunch", "drinks", "a walk". Empty means they didn't say, which is not the same as wanting nothing: an empty request keeps every stop the window allows. One named stop seeds a plan built around it; two or more describe the shape exactly. See `SlotSchedule.chooseBands`.
    var requestedStops: Set<SlotBand>
    /// The host said these are the *only* stops they want — "just dinner". Separate from `requestedStops` because one named stop cannot otherwise tell "an evening with dinner in it" apart from "dinner and nothing else", and those are different plans.
...
nonisolated struct OutingBrief: Sendable, Equatable {
    ...
    /// The stops the host named. One seeds a plan built around it; two or more describe it exactly — see `SlotSchedule.chooseBands`.
    ...
    /// What kind of night this is. Drives how many stops fill a gap, and how venues are ranked inside each — never which stops exist.
    ...
    /// The values Wandr filled in on the host's behalf. Drives the demo's "here's what we assumed" affordance without a new service contract.
...
nonisolated extension OutingBrief {
    ...
    /// The stops this brief's plan will contain, in time order. The *only* place `SlotSchedule.compute` is called from production code. Five separate call sites used to pass the brief's window and stops themselves — the curator, the fake curator, the evidence resolver, the schedule drafter and the poll builder — which meant a new input like `stopsAreExclusive` had to be threaded through five times to avoid the deck, the window label on the card, and the timeline quietly disagreeing about what the night is. Pure arithmetic over six bands, so recomputing it per caller costs nothing.
