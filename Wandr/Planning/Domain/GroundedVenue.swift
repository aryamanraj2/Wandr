// GroundedVenue.swift Wandr Grounded evidence. The model does not own any of this. An option may reach the curation UI only if it carries a stable ID, a source, a retrieval timestamp, a category, a known-or-explicitly-unknown cost, and an availability state. Everything here is an immutable snapshot.
...
nonisolated struct EvidenceSource: Sendable, Equatable, Hashable {
    ...
    // MapKit, live hours, and route providers are added here later. None of them may block the local demo path.
}
...
nonisolated enum VenueCost: Sendable, Equatable, Hashable {
    ...
    /// "Paisa Vasool" — pure arithmetic, `max(listPrice - perHead, 0)`. `nil` when either number is unknown. The model never calculates this.
...
/// A tag set the provider either established or didn't. The distinction matters: an *absent* vegetarian tag on a venue we surveyed is a contradiction, while an unsurveyed venue is merely unverified. One fails, the other warns.
...
nonisolated enum VenueSetting: String, Sendable, Equatable, Hashable, CaseIterable {
    ...
    /// Whether this venue satisfies an explicit host preference. Returns `nil` when the venue's setting was never established — unverified, not contradicted.
...
/// One immutable venue evidence snapshot. Nothing in here is generated by a model. Names, costs, offers, hours, and tags all come from the provider, and the curator may only reference these by `venueID`.
nonisolated struct GroundedVenue: Sendable, Equatable, Identifiable {
    ...
    /// Provider-stated offer text, e.g. "2-for-1 on cocktails". Never model prose, and never a claim the dataset didn't make. `nil` means no offer was stated.
    let offer: String?
    /// When the offer applies, as the provider labels it. A label only — never parsed into a `Date`, and never validated against the schedule in this step.
...
// `RouteEvidence` and `ForecastEvidence` are deliberately absent from this slice. MapKit routes and WeatherKit forecasts are deferred rules; defining empty shells now would invite faking live data. Their warnings already have a home in `PlanWarning`, which is the seam that matters.
