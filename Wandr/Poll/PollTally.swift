// PollTally.swift Wandr The lock rule, as a set of pure functions over a `SquadSlotPoll`. This is the single owner of "who won and are we done yet" — the plurality-max analogue of the Docs' `LockRule`. It reads a poll and never mutates one; deciding *when* to lock belongs to the session, which acts on this reading. Like `FeasibilityValidator`, nothing here touches a model, the network, disk, or a UI framework — which is the point. It is exercised entirely by `PollTallyTests`.
...
nonisolated enum PollTally {
    ...
    /// Whether everyone has voted. Quorum is met when the number of distinct voters reaches `size` (dedupe means each participant counts once).
    static func quorumReached(_ poll: SquadSlotPoll) -> Bool {
        poll.votes.count >= poll.size
    }

    /// The poll's standing. A locked poll reports its recorded winner. Otherwise the winner stays hidden until quorum, then it is the plurality leader — or a tie when several options share the top count.
